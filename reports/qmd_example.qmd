...
![Horse populations for all provinces in Canada from 1906 - 1972](../results/horse_pops_plot.png){#fig-horse_hist_pop width=70%}

We can see from @fig-horse_hist_pop that Ontario, Saskatchewan and Alberta 
...
Additionally, note that in @tbl-horses_sd we consider the sample standard deviation of the number of horses during the same time span as @fig-horse_hist_pop.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse_sd_pop width=70%}

In @fig-horse_sd_pop we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
